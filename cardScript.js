...
async function loadQuestionsData() {
  ...
  if (isDebug) {
    ...
    if (details.includes('on_on')) {
      fetchPromises.push(fetch('./on_on.json').then(res => res.json()));
    }
    if (details.includes('kun_kun')) {
      fetchPromises.push(fetch('./kun_kun.json').then(res => res.json()));
    }
    if (details.includes('on_kun')) {
      fetchPromises.push(fetch('./on_kun.json').then(res => res.json()));
    }
    if (details.includes('kun_on')) {
      fetchPromises.push(fetch('./kun_on.json').then(res => res.json()));
    }
    if (details.includes('jukujikun')) {
      fetchPromises.push(fetch('./jukujikun.json').then(res => res.json()));
    }
    if (details.includes('multi')) {
      fetchPromises.push(fetch('./multi.json').then(res => res.json()));
    }
